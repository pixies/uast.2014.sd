#Atividade:
##Faça uma revisão sistemática utilizando 5 autores, sobre os temas a baixo.

Escolha e monte o tema da sua pesquisa da seguinte forma:

##Exemplo:
	1 (Modelo) Área Geral + TEMA + uma Revisão Sistemática
	2 (Exemplo) Tolerância a Falha em (Sistemas ou Clusters) de balanceamento de Redes, \numa Revisão Sistemática. 

##Utilize os slides para servir de base sobre o que pesquisar:
	1. SD - 7 - Comunicação entre Processos - Revisao.ppt
	2. SD - 8 - Atomicidade e Concorrencia.pdf
	3. SD - 8 - Tolerancia a Falhas.pdf

##Área Geral:
	1. Comunicação entre processos;
	2. Tolerância a Falhas;
	3. Atomicidade e Concorrência;

##sub-area:

	1. Cluster de Alto desempenho
		1.1 HPC (High Performance Computing) (TEMAS)
		1.2 DPC (Distributed/Parallel Computing) (TEMAS)
	2. Alta disponibilidade
		1.1 Clusterização (TEMAS)
	3. Cluster para balanceamento de carga 
		3.1 Storage(Balanceamento de armazenamento) (TEMAS)
		3.2 Balanceamento de Rede (TEMAS)
		3.3 Balanceamento de CPU (TEMAS)
